...
import { makeEmbedder } from '#/services/matcher/embedder'
...
async function main(): Promise<void> {
  ...
  console.log(
    `thesis pages: ${thesisExtract.pages.length}, unrelated pages: ${unrelatedExtract.pages.length}`,
  )

  const citations = groupCitations(parseCitationsFromPages(thesisExtract.pages))
