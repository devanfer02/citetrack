...
// Shared per-host throttle for any outbound HTTP we make: KBBI scrapes
// today, autofetch provider lookups + PDF downloads now too. State
// lives in process memory, so a single Node process serializes all
// jobs hitting the same host through one FIFO queue with a minimum
// gap. When a host returns 429/503, pauseHost() honors Retry-After
// and isHostPaused() lets callers skip to the next provider in their
// fallback chain.
