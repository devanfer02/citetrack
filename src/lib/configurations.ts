...
// Bilingual search keywords ("meta tags") per setting. These never render —
// they only widen what the settings search box can match, so a student typing
// "ukuran unggah" or "upload size" lands on the same card. Keep them lowercase;
// the matcher lowercases the query before comparing.
export const CONFIG_KEYWORDS: Record<ConfigKey, readonly string[]> = {
  'autofetch.staleness_timeout_ms': [
    'pencarian otomatis',
    'macet',
    'diam',
